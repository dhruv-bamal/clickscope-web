"use client";
// Own useState (hover index) + useId() (gradient id) — an interactive chart,
// not a consequence of anything else in the library.
...
export function AnalyticsChart({ data = [], labels, height = 240, loading = false, valueFormat }: AnalyticsChartProps) {
  ...
  const ny = (v: number) => pad + (1 - (v - min) / (max - min || 1)) * plotH;
  ...
  return (
    <div style={{ position: "relative" }}>
      ...
      >
        ...
        <text x={pad} y={height - 4} fontSize="11" fill="var(--color-fg-subtle)">
          {ticks[0]}
        </text>
        <text x={w / 2} y={height - 4} fontSize="11" textAnchor="middle" fill="var(--color-fg-subtle)">
          {ticks[Math.floor(ticks.length / 2)]}
        </text>
        <text x={w - pad} y={height - 4} fontSize="11" textAnchor="end" fill="var(--color-fg-subtle)">
      ...
      {hover != null && (
        ...
        >
          <div style={{ fontWeight: 600 }} className="cs-tnum">
            {fmt(data[hover])}
          </div>
